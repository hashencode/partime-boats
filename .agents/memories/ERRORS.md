...
- 2026-06-12（禁用列表虚拟滚动）：`bun run test src/shared/template-kit/recipes/standard-list-page-recipe.test.tsx src/pages/templates/msk-query-list/msk-query-list-page.test.tsx` 会被 Maersk 页面既有用例 `should block batch submit when no fields are changed` 5s 超时拦住；本次相关用例可用精确 `--testNamePattern` 单独验证，避免把旧异步清理问题误判为虚拟滚动回归。
- 2026-08-11（Maersk 航线映射）：新环境先执行 `bun install --frozen-lockfile` 才有本地 `rstest`；安装后若报 `@rspack/core@2.0.0-rc.1` 与 binding `1.7.11` 不匹配，可将 `RSPACK_BINDING` 指向 `node_modules/@rstest/core/node_modules/@rsbuild/core/node_modules/@rspack/core/node_modules/@rspack/binding` 后运行定向测试。 #promote

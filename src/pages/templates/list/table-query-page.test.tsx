...
describe('TableQueryPage', () => {
  ...
  it('shows partial state when list response is partial', async () => {
    server.use(
      http.get('*/api/template/rules', () =>
        HttpResponse.json({
          data: [{ key: '1', name: 'TradeCode X', desc: 'partial', callNo: 1, status: 1, updatedAt: new Date().toISOString() }],
          partial: true,
    ...
    server.use(
      http.get('*/api/template/rules', () => {
        ...
        return HttpResponse.json({
          data: [{ key: '1', name: 'TradeCode A', desc: 'default', callNo: 1, status: 1, updatedAt: new Date().toISOString() }],
          total: 1,
          success: true,
        })
      })

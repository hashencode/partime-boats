...
const createBaseSpec = (form: ReturnType<typeof Form.useForm<DemoValues>>[0]): BasicCrudFormSpec<DemoValues> => ({
  ...
  title: '编辑演示',
  stateCopy: {
    readonlyNotice: '当前为查看模式，表单内容仅可查看，不可编辑或保存。',
...
describe('BasicCrudFormRecipe', () => {
  ...
  it('renders readonly notice and hides actions in readonly mode', () => {
    const Demo = () => {
      const [form] = Form.useForm<DemoValues>()
      const spec: BasicCrudFormSpec<DemoValues> = {
        ...createBaseSpec(form),
    ...
    expect(screen.getByText('当前为查看模式，表单内容仅可查看，不可编辑或保存。')).toBeTruthy()
    expect(screen.queryByRole('button', { name: '保存' })).toBeNull()
    expect(screen.getByPlaceholderText('请输入名称')).toBeTruthy()
  })

  it('calls resetAll from the unified reset action', () => {
    let resetCount = 0
